...
def migrate_db():
    ...
    migrations = [
        ...
        "ALTER TABLE user_settings ADD COLUMN sync_interval INTEGER DEFAULT 5",
...
async def update_settings(s: SettingsUpdate, current_user: dict = Depends(get_current_user)):
    conn = get_db()
    # IMPORTANT: this used to be `INSERT OR REPLACE INTO user_settings (...)`
    # — which deletes the whole existing row and re-inserts one with ONLY
    # the columns listed here. Any column NOT in that list (gas_url,
    # sheet_id, and any future column added later) silently got reset to
    # its default on every single settings save. That was the actual cause
    # of "settings keep disappearing / have to redo everything after
    # login" — specifically it kept wiping the Google Sheet sync URL.
    # UPDATE only ever touches the columns explicitly listed below, so
    # everything else on the row is left exactly as it was.
    conn.execute("INSERT OR IGNORE INTO user_settings (user_id) VALUES (?)", (current_user["id"],))
    conn.execute("""UPDATE user_settings SET
        shop_name=?, address=?, mobile=?, upi_id=?, gst_number=?, fssai_no=?, extra_header_lines=?,
        ...
        WHERE user_id=?""",
        (s.shop_name, s.address, s.mobile, s.upi_id, s.gst_number,
         ...
         s.waiter_list, s.table_list, current_user["id"]))
    conn.commit()
    conn.close()
    return {"success": True}

class CloudConfigUpdate(BaseModel):
    gas_url: Optional[str] = ""
    sheet_id: Optional[str] = ""
    sync_interval: Optional[int] = 5

@app.put("/api/settings/cloud")
async def update_cloud_config(s: CloudConfigUpdate, current_user: dict = Depends(get_current_user)):
    """User self-service save for the Settings → Cloud tab (Google Sheet
    sync URL/ID/interval). This endpoint didn't exist before — the Cloud
    tab's "Save Cloud Config" button only ever wrote to localStorage, so
    the config was lost on logout (clearPerUserCache), on a new device, or
    if the browser data was cleared, with no way to recover it. Admins have
    a separate /api/admin/update-gas-url for setting it on a user's behalf;
    this is the equivalent for the user themselves."""
    conn = get_db()
    conn.execute("INSERT OR IGNORE INTO user_settings (user_id) VALUES (?)", (current_user["id"],))
    conn.execute(
        "UPDATE user_settings SET gas_url=?, sheet_id=?, sync_interval=? WHERE user_id=?",
        (s.gas_url or '', s.sheet_id or '', s.sync_interval or 5, current_user["id"])
    )
    conn.commit()
    conn.close()
    return {"success": True}
